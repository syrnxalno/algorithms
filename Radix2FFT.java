        ...
        for (int stage = 1; stage <= stages; stage++) {  
            ...
            for (int j = 0; j < BWidth; j++) {  
                ...
                for (int HiIndex = j; HiIndex < N; HiIndex += BSep) {  
                    ...
                    if (j != 0) {  
                        Complex TEMP = pLo.multiply(WN);  
                        pLo.re = TEMP.re;  
                        pLo.im = TEMP.im;  
                    }  

                    double tempRe = pLo.re;  
                    double tempIm = pLo.im;  
                    DFT[HiIndex + BWidth].re = pHi.re - tempRe;  
                    DFT[HiIndex + BWidth].im = pHi.im - tempIm;  
                    DFT[HiIndex].re = pHi.re + tempRe;  
                    DFT[HiIndex].im = pHi.im + tempIm;  
                }  
            }  
        }  
    }  
